...
- `use_settings_file` if set to `1` uses parameters in `train/<YOUR_CHOSEN_MODEL>_settings.json` (which is created by the hyper paramter optimization run, see `run_hpo` above). Setting this to `1` disregards any hyperparameters in the config structure. Otherwise hyperparamters from `optimal_hpo_settings` in the config structure under respective variant will be used.
